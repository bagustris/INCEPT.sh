# 518 — `ipcs` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`ipcs`** command in ChatML JSONL format.
...
-m -q -s -a -l --shmems --queues --semaphores --all --limits

Cover ALL documented flags and options for `ipcs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ipcs>"}, {"role": "assistant", "content": "<correct ipcs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ipcs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ipcs to...")
