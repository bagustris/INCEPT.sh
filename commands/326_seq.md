# 326 — `seq` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`seq`** command in ChatML JSONL format.
...
-w -f -s --equal-width --format --separator

Cover ALL documented flags and options for `seq`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for seq>"}, {"role": "assistant", "content": "<correct seq command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<seq with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use seq to...")
