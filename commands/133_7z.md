# 133 — `7z` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`7z`** command in ChatML JSONL format.
...
a x l t e -p -o -r -y -mx= -mmt= -v add extract list test

Cover ALL documented flags and options for `7z`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for 7z>"}, {"role": "assistant", "content": "<correct 7z command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<7z with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use 7z to...")
