# 124 — `gunzip` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`gunzip`** command in ChatML JSONL format.
...
-k -v -f -r -c --keep --verbose --force --recursive --stdout

Cover ALL documented flags and options for `gunzip`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for gunzip>"}, {"role": "assistant", "content": "<correct gunzip command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<gunzip with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use gunzip to...")
