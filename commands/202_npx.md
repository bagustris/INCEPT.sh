# 202 — `npx` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`npx`** command in ChatML JSONL format.
...
-p -c --package --call --yes --no

Cover ALL documented flags and options for `npx`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for npx>"}, {"role": "assistant", "content": "<correct npx command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<npx with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use npx to...")
