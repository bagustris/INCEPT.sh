# 280 — `firewall-cmd` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`firewall-cmd`** command in ChatML JSONL format.

## Command Overview

`firewall-cmd` — Firewalld control tool
...
--add-port --remove-port --add-service --remove-service --add-rich-rule --list-all --list-ports --list-services --reload --permanent --zone --get-zones --get-active-zones --get-default-zone --set-default-zone --query-port --query-service --info-zone

Cover ALL documented flags and options for `firewall-cmd`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for firewall-cmd>"}, {"role": "assistant", "content": "<correct firewall-cmd command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<firewall-cmd with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use firewall-cmd to...")
