# 184 — `brctl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`brctl`** command in ChatML JSONL format.
...
addbr delbr addif delif show setageing setbridgeprio setfd sethello setmaxage setpathcost setportprio stp

Cover ALL documented flags and options for `brctl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for brctl>"}, {"role": "assistant", "content": "<correct brctl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<brctl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use brctl to...")
