# 355 — `objdump` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`objdump`** command in ChatML JSONL format.
...
-d -D -h -t -T -r -R -s -x -f --disassemble --disassemble-all --headers --syms --dynamic-syms --reloc --dynamic-reloc --full-contents --all-headers --file-headers

Cover ALL documented flags and options for `objdump`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for objdump>"}, {"role": "assistant", "content": "<correct objdump command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<objdump with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use objdump to...")
