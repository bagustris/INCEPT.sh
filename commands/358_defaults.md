# 358 — `defaults` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`defaults`** command in ChatML JSONL format.
...
read write delete read-type domains find -g -currentHost -app

Cover ALL documented flags and options for `defaults`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for defaults>"}, {"role": "assistant", "content": "<correct defaults command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<defaults with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use defaults to...")
