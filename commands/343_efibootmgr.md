# 343 — `efibootmgr` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`efibootmgr`** command in ChatML JSONL format.

## Command Overview

`efibootmgr` — Manage UEFI boot entries
**Category:** Kernel and Hardware
...
-v -c -d -p -L -l -b -B -o --verbose --create --disk --part --label --loader --bootnum --delete-bootnum --bootorder

Cover ALL documented flags and options for `efibootmgr`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for efibootmgr>"}, {"role": "assistant", "content": "<correct efibootmgr command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<efibootmgr with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use efibootmgr to...")
