# 093 — `split` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`split`** command in ChatML JSONL format.
...
-b -l -n -d -a --bytes --lines --number --suffix-length

Cover ALL documented flags and options for `split`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for split>"}, {"role": "assistant", "content": "<correct split command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<split with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use split to...")
