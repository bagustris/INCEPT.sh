# 042 — `shutdown` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`shutdown`** command in ChatML JSONL format.

## Command Overview

`shutdown` — Halt power-off or reboot
...
-h -r -c -k now +minutes HH:MM --halt --reboot --cancel

Cover ALL documented flags and options for `shutdown`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for shutdown>"}, {"role": "assistant", "content": "<correct shutdown command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<shutdown with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use shutdown to...")
