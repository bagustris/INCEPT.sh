# 356 — `strip` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`strip`** command in ChatML JSONL format.
...
-s -g -x --strip-all --strip-debug --strip-unneeded

Cover ALL documented flags and options for `strip`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for strip>"}, {"role": "assistant", "content": "<correct strip command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<strip with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use strip to...")
