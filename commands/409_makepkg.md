# 409 — `makepkg` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`makepkg`** command in ChatML JSONL format.

## Command Overview

`makepkg` — Make Arch Linux packages
...
-s -i -c -f -d --syncdeps --install --clean --force --nodeps --noextract --nocheck

Cover ALL documented flags and options for `makepkg`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for makepkg>"}, {"role": "assistant", "content": "<correct makepkg command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<makepkg with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use makepkg to...")
