# 084 — `su` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`su`** command in ChatML JSONL format.
...
- -l -c -s --login --command --shell

Cover ALL documented flags and options for `su`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for su>"}, {"role": "assistant", "content": "<correct su command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<su with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use su to...")
