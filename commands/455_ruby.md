# 455 — `ruby` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`ruby`** command in ChatML JSONL format.
...
-e -n -p -i -l -a -w --eval --in-place --line --autosplit --warnings

Cover ALL documented flags and options for `ruby`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ruby>"}, {"role": "assistant", "content": "<correct ruby command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ruby with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ruby to...")
