# 556 — `chronyc` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`chronyc`** command in ChatML JSONL format.
...
tracking sources sourcestats activity makestep burst online offline

Cover ALL documented flags and options for `chronyc`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for chronyc>"}, {"role": "assistant", "content": "<correct chronyc command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<chronyc with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use chronyc to...")
