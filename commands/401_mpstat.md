# 401 — `mpstat` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`mpstat`** command in ChatML JSONL format.
...
-P -u -A -I -o --processor --cpu --all --interrupts --json

Cover ALL documented flags and options for `mpstat`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for mpstat>"}, {"role": "assistant", "content": "<correct mpstat command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<mpstat with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use mpstat to...")
