# 142 — `hwclock` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`hwclock`** command in ChatML JSONL format.
...
--show --set --systohc --hctosys --utc --localtime --verbose

Cover ALL documented flags and options for `hwclock`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for hwclock>"}, {"role": "assistant", "content": "<correct hwclock command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<hwclock with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use hwclock to...")
