# 493 — `rabbitmqctl` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`rabbitmqctl`** command in ChatML JSONL format.
...
status list_queues list_exchanges list_bindings list_connections list_channels add_user delete_user set_permissions clear_permissions set_policy cluster_status join_cluster

Cover ALL documented flags and options for `rabbitmqctl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for rabbitmqctl>"}, {"role": "assistant", "content": "<correct rabbitmqctl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<rabbitmqctl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use rabbitmqctl to...")
