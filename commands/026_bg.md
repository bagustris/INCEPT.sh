# 026 — `bg` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`bg`** command in ChatML JSONL format.
...
Cover ALL documented flags and options for `bg`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for bg>"}, {"role": "assistant", "content": "<correct bg command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<bg with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use bg to...")
