# 223 — `setfacl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`setfacl`** command in ChatML JSONL format.
...
-m -x -b -k -R -d -n --modify --remove --remove-all --remove-default --recursive --default --no-mask --set

Cover ALL documented flags and options for `setfacl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for setfacl>"}, {"role": "assistant", "content": "<correct setfacl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<setfacl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use setfacl to...")
