# 550 — `spark-submit` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`spark-submit`** command in ChatML JSONL format.
...
--master --deploy-mode --class --driver-memory --executor-memory --num-executors --conf --packages --jars --py-files

Cover ALL documented flags and options for `spark-submit`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for spark-submit>"}, {"role": "assistant", "content": "<correct spark-submit command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<spark-submit with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use spark-submit to...")
