# 178 — `iperf3` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`iperf3`** command in ChatML JSONL format.
...
-s -c -t -p -P -R -u -b -n -J --server --client --time --port --parallel --reverse --udp --bandwidth --bytes --json

Cover ALL documented flags and options for `iperf3`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for iperf3>"}, {"role": "assistant", "content": "<correct iperf3 command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<iperf3 with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use iperf3 to...")
