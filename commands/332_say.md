# 332 — `say` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`say`** command in ChatML JSONL format.
...
-v -r -o --voice --rate --output-file

Cover ALL documented flags and options for `say`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for say>"}, {"role": "assistant", "content": "<correct say command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<say with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use say to...")
