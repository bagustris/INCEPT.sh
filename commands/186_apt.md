# 186 — `apt` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`apt`** command in ChatML JSONL format.
...
install remove purge update upgrade full-upgrade autoremove search show list edit-sources --fix-broken -y -d --download-only --no-install-recommends --reinstall

Cover ALL documented flags and options for `apt`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for apt>"}, {"role": "assistant", "content": "<correct apt command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<apt with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use apt to...")
