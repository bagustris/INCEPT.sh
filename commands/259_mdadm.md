# 259 — `mdadm` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`mdadm`** command in ChatML JSONL format.

## Command Overview

`mdadm` — Manage MD/software RAID
...
--create --assemble --manage --detail --examine --monitor --grow --stop --add --remove --fail -l --level -n --raid-devices -x --spare-devices

Cover ALL documented flags and options for `mdadm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for mdadm>"}, {"role": "assistant", "content": "<correct mdadm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<mdadm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use mdadm to...")
