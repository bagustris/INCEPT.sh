# 583 — `ipconfig` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`ipconfig`** command in ChatML JSONL format.

## Command Overview

`ipconfig` — IP configuration (macOS/Windows)
...
getifaddr getpacket getsummary set waitall

Cover ALL documented flags and options for `ipconfig`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ipconfig>"}, {"role": "assistant", "content": "<correct ipconfig command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ipconfig with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ipconfig to...")
