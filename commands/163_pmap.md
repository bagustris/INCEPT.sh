# 163 — `pmap` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pmap`** command in ChatML JSONL format.
...
-x -d -q -A --extended --device --quiet --range

Cover ALL documented flags and options for `pmap`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pmap>"}, {"role": "assistant", "content": "<correct pmap command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pmap with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pmap to...")
