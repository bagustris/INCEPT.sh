# 187 — `apt-get` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`apt-get`** command in ChatML JSONL format.
...
install remove purge update upgrade dist-upgrade autoremove autoclean clean download source build-dep -y -f -d --reinstall --no-install-recommends

Cover ALL documented flags and options for `apt-get`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for apt-get>"}, {"role": "assistant", "content": "<correct apt-get command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<apt-get with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use apt-get to...")
