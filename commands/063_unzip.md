# 063 — `unzip` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`unzip`** command in ChatML JSONL format.

## Command Overview

`unzip` — Extract compressed files from ZIP archive
...
-l -t -o -d -q -v -n --list --test --overwrite --directory --quiet

Cover ALL documented flags and options for `unzip`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for unzip>"}, {"role": "assistant", "content": "<correct unzip command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<unzip with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use unzip to...")
