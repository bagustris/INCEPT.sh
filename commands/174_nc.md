# 174 — `nc` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nc`** command in ChatML JSONL format.
...
-l -p -u -v -z -w -k -n --listen --port --udp --verbose --zero --wait --keep-open --numeric

Cover ALL documented flags and options for `nc`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nc>"}, {"role": "assistant", "content": "<correct nc command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nc with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nc to...")
