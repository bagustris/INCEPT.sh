# 214 — `chage` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`chage`** command in ChatML JSONL format.
...
-l -d -E -I -m -M -W --list --lastday --expiredate --inactive --mindays --maxdays --warndays

Cover ALL documented flags and options for `chage`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for chage>"}, {"role": "assistant", "content": "<correct chage command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<chage with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use chage to...")
