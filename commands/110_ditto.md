# 110 — `ditto` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ditto`** command in ChatML JSONL format.

## Command Overview

`ditto` — Copy directory hierarchies (macOS)
...
-v -V -X -k --rsrc --extattr

Cover ALL documented flags and options for `ditto`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ditto>"}, {"role": "assistant", "content": "<correct ditto command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ditto with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ditto to...")
