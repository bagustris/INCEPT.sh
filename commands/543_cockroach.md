# 543 — `cockroach` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`cockroach`** command in ChatML JSONL format.

## Command Overview

`cockroach` — CockroachDB
**Category:** Graph and Time-Series Databases
...
start start-single-node init cert sql dump node quit --insecure --listen-addr --join --store --certs-dir

Cover ALL documented flags and options for `cockroach`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cockroach>"}, {"role": "assistant", "content": "<correct cockroach command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cockroach with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cockroach to...")
