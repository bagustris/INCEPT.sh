# 237 — `lsblk` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`lsblk`** command in ChatML JSONL format.
...
-a -f -l -o -p -r -t -J -n --all --fs --list --output --paths --raw --topology --json --noheadings

Cover ALL documented flags and options for `lsblk`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lsblk>"}, {"role": "assistant", "content": "<correct lsblk command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lsblk with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lsblk to...")
