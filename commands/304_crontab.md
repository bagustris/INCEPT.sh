# 304 — `crontab` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`crontab`** command in ChatML JSONL format.
...
-e -l -r -u -i --edit --list --remove --user --interactive

Cover ALL documented flags and options for `crontab`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for crontab>"}, {"role": "assistant", "content": "<correct crontab command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<crontab with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use crontab to...")
