# 022 — `kill` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`kill`** command in ChatML JSONL format.
...
-9 -15 -TERM -KILL -HUP -USR1 -USR2 -CONT -STOP -INT -l --signal

Cover ALL documented flags and options for `kill`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for kill>"}, {"role": "assistant", "content": "<correct kill command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<kill with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use kill to...")
