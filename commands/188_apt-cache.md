# 188 — `apt-cache` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`apt-cache`** command in ChatML JSONL format.

## Command Overview

`apt-cache` — APT package cache query
...
search show showpkg depends rdepends policy pkgnames stats unmet --names-only --full

Cover ALL documented flags and options for `apt-cache`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for apt-cache>"}, {"role": "assistant", "content": "<correct apt-cache command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<apt-cache with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use apt-cache to...")
