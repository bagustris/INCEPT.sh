# 468 — `gradle` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`gradle`** command in ChatML JSONL format.

## Command Overview

`gradle` — Build automation tool
**Category:** Build Tools and Task Runners
...
build clean test run assemble check dependencies -x --info --debug --stacktrace --parallel --daemon

Cover ALL documented flags and options for `gradle`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for gradle>"}, {"role": "assistant", "content": "<correct gradle command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<gradle with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use gradle to...")
