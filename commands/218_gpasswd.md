# 218 — `gpasswd` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`gpasswd`** command in ChatML JSONL format.
...
-a -d -r -R -A -M --add --delete --remove-password --restrict --administrators --members

Cover ALL documented flags and options for `gpasswd`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for gpasswd>"}, {"role": "assistant", "content": "<correct gpasswd command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<gpasswd with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use gpasswd to...")
