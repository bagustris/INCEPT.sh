# 251 — `smartctl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`smartctl`** command in ChatML JSONL format.

## Command Overview

`smartctl` — Control SMART-enabled drives
...
-a -i -H -t -l -A --all --info --health --test --log --attributes short long conveyance offline

Cover ALL documented flags and options for `smartctl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for smartctl>"}, {"role": "assistant", "content": "<correct smartctl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<smartctl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use smartctl to...")
