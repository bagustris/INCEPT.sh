# 169 — `ssh-keyscan` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ssh-keyscan`** command in ChatML JSONL format.
...
-t -p -T -H --type --port --timeout --hash

Cover ALL documented flags and options for `ssh-keyscan`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ssh-keyscan>"}, {"role": "assistant", "content": "<correct ssh-keyscan command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ssh-keyscan with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ssh-keyscan to...")
