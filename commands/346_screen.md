# 346 — `screen` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`screen`** command in ChatML JSONL format.
...
-S -r -d -ls -X -L -h -wipe -R -DR attach detach quit split focus select title caption

Cover ALL documented flags and options for `screen`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for screen>"}, {"role": "assistant", "content": "<correct screen command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<screen with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use screen to...")
