# 566 — `rackup` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`rackup`** command in ChatML JSONL format.

## Command Overview

`rackup` — Rack web server launcher
...
-p -o -E -D --port --host --env --daemonize

Cover ALL documented flags and options for `rackup`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for rackup>"}, {"role": "assistant", "content": "<correct rackup command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<rackup with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use rackup to...")
