# 180 — `ethtool` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ethtool`** command in ChatML JSONL format.

## Command Overview

`ethtool` — Query or control network driver settings
**Category:** Networking
...
-i -a -A -c -C -g -G -k -K -s -S --driver --show-pause --pause --show-coalesce --coalesce --show-ring --ring --show-offload --offload --change --statistics

Cover ALL documented flags and options for `ethtool`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ethtool>"}, {"role": "assistant", "content": "<correct ethtool command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ethtool with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ethtool to...")
