# 454 — `node` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`node`** command in ChatML JSONL format.
...
-e -p -r -v --eval --print --require --version --inspect --inspect-brk

Cover ALL documented flags and options for `node`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for node>"}, {"role": "assistant", "content": "<correct node command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<node with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use node to...")
