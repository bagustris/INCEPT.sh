# 119 — `emacs` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`emacs`** command in ChatML JSONL format.
...
-nw -q -Q --no-window-system --no-init-file --quick --batch -f --funcall --eval

Cover ALL documented flags and options for `emacs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for emacs>"}, {"role": "assistant", "content": "<correct emacs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<emacs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use emacs to...")
