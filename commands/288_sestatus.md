# 288 — `sestatus` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`sestatus`** command in ChatML JSONL format.
...
-v -b --verbose --booleans

Cover ALL documented flags and options for `sestatus`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sestatus>"}, {"role": "assistant", "content": "<correct sestatus command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sestatus with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sestatus to...")
