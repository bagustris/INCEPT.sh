# 092 — `tree` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`tree`** command in ChatML JSONL format.
...
-L -d -a -f -i -p -s -h --dirsfirst -I -P --prune -o --charset

Cover ALL documented flags and options for `tree`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for tree>"}, {"role": "assistant", "content": "<correct tree command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<tree with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use tree to...")
