# 010 — `chattr` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`chattr`** command in ChatML JSONL format.

## Command Overview

`chattr` — Change file attributes on ext filesystem
...
+i -i +a -a +d -d +e -e +s -s -R -V --recursive --verbose

Cover ALL documented flags and options for `chattr`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for chattr>"}, {"role": "assistant", "content": "<correct chattr command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<chattr with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use chattr to...")
