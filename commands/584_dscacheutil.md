# 584 — `dscacheutil` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`dscacheutil`** command in ChatML JSONL format.

## Command Overview

`dscacheutil` — Directory Service cache (macOS)
...
-q -flushcache --query --cachedump

Cover ALL documented flags and options for `dscacheutil`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dscacheutil>"}, {"role": "assistant", "content": "<correct dscacheutil command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dscacheutil with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dscacheutil to...")
