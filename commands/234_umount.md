# 234 — `umount` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`umount`** command in ChatML JSONL format.
...
-f -l -a -r -v --force --lazy --all --read-only --verbose

Cover ALL documented flags and options for `umount`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for umount>"}, {"role": "assistant", "content": "<correct umount command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<umount with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use umount to...")
