# 118 — `jq` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`jq`** command in ChatML JSONL format.
...
. .field .[] select map keys values length type to_entries from_entries group_by sort_by unique_by flatten -r -c -e -S --raw-output --compact-output --slurp --arg --argjson

Cover ALL documented flags and options for `jq`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for jq>"}, {"role": "assistant", "content": "<correct jq command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<jq with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use jq to...")
