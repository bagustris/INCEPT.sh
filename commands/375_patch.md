# 375 — `patch` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`patch`** command in ChatML JSONL format.
...
-p -R -d -i -o --dry-run --verbose --reverse --strip

Cover ALL documented flags and options for `patch`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for patch>"}, {"role": "assistant", "content": "<correct patch command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<patch with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use patch to...")
