# 081 — `traceroute` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`traceroute`** command in ChatML JSONL format.
...
5. **Include piped commands** where natural (at least 20 examples):
   - `traceroute ... | grep ...`
   - `traceroute ... | sort | uniq`
   - `traceroute ... | head/tail/wc -l`
   - `traceroute ... | awk ...`
...
-n -m -w -q -I -T -U -p -4 -6 --numeric --max-hops --wait --queries --icmp --tcp --udp --port

Cover ALL documented flags and options for `traceroute`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for traceroute>"}, {"role": "assistant", "content": "<correct traceroute command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<traceroute with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use traceroute to...")
