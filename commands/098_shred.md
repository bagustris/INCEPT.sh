# 098 — `shred` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`shred`** command in ChatML JSONL format.
...
-n -z -u -v -f --remove --zero --iterations

Cover ALL documented flags and options for `shred`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for shred>"}, {"role": "assistant", "content": "<correct shred command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<shred with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use shred to...")
