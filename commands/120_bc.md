# 120 — `bc` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`bc`** command in ChatML JSONL format.
...
-l -q -s --mathlib --quiet --standard scale ibase obase

Cover ALL documented flags and options for `bc`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for bc>"}, {"role": "assistant", "content": "<correct bc command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<bc with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use bc to...")
