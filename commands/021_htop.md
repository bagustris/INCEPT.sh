# 021 — `htop` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`htop`** command in ChatML JSONL format.
...
-d -u -p -s -t --delay --user --pid --sort-key --tree

Cover ALL documented flags and options for `htop`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for htop>"}, {"role": "assistant", "content": "<correct htop command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<htop with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use htop to...")
