# 190 — `dpkg` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`dpkg`** command in ChatML JSONL format.
...
-i -r -P -l -L -s -S -c --install --remove --purge --list --listfiles --status --search --contents --configure --audit

Cover ALL documented flags and options for `dpkg`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dpkg>"}, {"role": "assistant", "content": "<correct dpkg command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dpkg with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dpkg to...")
