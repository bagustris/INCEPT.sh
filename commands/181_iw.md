# 181 — `iw` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`iw`** command in ChatML JSONL format.
...
dev phy list info link scan connect disconnect reg set

Cover ALL documented flags and options for `iw`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for iw>"}, {"role": "assistant", "content": "<correct iw command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<iw with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use iw to...")
