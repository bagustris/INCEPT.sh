# 151 — `pstree` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pstree`** command in ChatML JSONL format.
...
-a -c -h -l -n -p -s -u -H --arguments --compact --highlight --long --numeric-sort --show-pids --show-parents --uid-changes

Cover ALL documented flags and options for `pstree`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pstree>"}, {"role": "assistant", "content": "<correct pstree command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pstree with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pstree to...")
