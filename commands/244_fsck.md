# 244 — `fsck` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`fsck`** command in ChatML JSONL format.
...
-t -A -a -y -n -r -v --type --all --automatic --yes --no --repair --verbose

Cover ALL documented flags and options for `fsck`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for fsck>"}, {"role": "assistant", "content": "<correct fsck command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<fsck with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use fsck to...")
