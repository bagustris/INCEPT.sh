# 262 — `hdiutil` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`hdiutil`** command in ChatML JSONL format.

## Command Overview

`hdiutil` — Disk image utility (macOS)
...
create mount unmount attach detach info compact convert verify burn

Cover ALL documented flags and options for `hdiutil`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for hdiutil>"}, {"role": "assistant", "content": "<correct hdiutil command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<hdiutil with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use hdiutil to...")
