# 069 — `df` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`df`** command in ChatML JSONL format.
...
- `-h`: Print sizes in human-readable format using powers of 1024
- `-T`: Display the filesystem type for each mounted filesystem
- `--output`: Select which columns to display (source, fstype, size, used, avail, pcent, target, etc.)

## Additional Flags to Cover

-h -i -T -a -l --human-readable --inodes --print-type --all --local --output --total --type --exclude-type

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for df>"}, {"role": "assistant", "content": "<correct df command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<df with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use df to...")
