# 273 — `nsenter` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nsenter`** command in ChatML JSONL format.
...
-t -m -u -i -n -p -U --target --mount --uts --ipc --net --pid --user

Cover ALL documented flags and options for `nsenter`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nsenter>"}, {"role": "assistant", "content": "<correct nsenter command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nsenter with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nsenter to...")
