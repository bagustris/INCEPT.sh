# 062 — `zip` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`zip`** command in ChatML JSONL format.
...
-r -e -x -9 -0 -j -q -v -u -d --recurse-paths --encrypt --exclude --compression-method

Cover ALL documented flags and options for `zip`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for zip>"}, {"role": "assistant", "content": "<correct zip command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<zip with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use zip to...")
