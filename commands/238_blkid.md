# 238 — `blkid` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`blkid`** command in ChatML JSONL format.
...
-o -s -t -p -L -U --output --match-tag --match-token --probe --label --uuid

Cover ALL documented flags and options for `blkid`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for blkid>"}, {"role": "assistant", "content": "<correct blkid command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<blkid with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use blkid to...")
