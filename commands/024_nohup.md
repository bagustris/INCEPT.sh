# 024 — `nohup` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`nohup`** command in ChatML JSONL format.
...
Cover ALL documented flags and options for `nohup`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nohup>"}, {"role": "assistant", "content": "<correct nohup command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nohup with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nohup to...")
