# 032 — `ss` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`ss`** command in ChatML JSONL format.
...
- `-p`: Show process information using the socket (PID and process name)
- `-a`: Display both listening and non-listening (established) sockets
- `-4`: Display only IPv4 sockets
- `-6`: Display only IPv6 sockets

## Additional Flags to Cover

-t -u -l -n -p -a -4 -6 -s -e -o -i --tcp --udp --listening --numeric --processes --all --ipv4 --ipv6 --summary --extended --options --info state sport dport

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ss>"}, {"role": "assistant", "content": "<correct ss command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ss with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ss to...")
