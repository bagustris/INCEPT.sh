# 130 — `zgrep` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`zgrep`** command in ChatML JSONL format.
...
-i -v -c -l -n -E -h --count --files-with-matches --line-number

Cover ALL documented flags and options for `zgrep`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for zgrep>"}, {"role": "assistant", "content": "<correct zgrep command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<zgrep with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use zgrep to...")
