# 426 — `log` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`log`** command in ChatML JSONL format.
...
show stream --predicate --info --debug --style --source --process --start --end --last

Cover ALL documented flags and options for `log`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for log>"}, {"role": "assistant", "content": "<correct log command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<log with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use log to...")
