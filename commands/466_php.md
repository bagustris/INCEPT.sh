# 466 — `php` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`php`** command in ChatML JSONL format.
...
-r -f -S -i -m -v --run --file --server --info --modules --version

Cover ALL documented flags and options for `php`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for php>"}, {"role": "assistant", "content": "<correct php command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<php with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use php to...")
