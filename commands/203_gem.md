# 203 — `gem` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`gem`** command in ChatML JSONL format.
...
install uninstall update list search info environment cleanup build -v --version --no-document --user-install --pre

Cover ALL documented flags and options for `gem`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for gem>"}, {"role": "assistant", "content": "<correct gem command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<gem with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use gem to...")
