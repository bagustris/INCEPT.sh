# 003 — `wc` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`wc`** command in ChatML JSONL format.
...
-l -w -c -m -L --lines --words --chars --bytes --max-line-length

Cover ALL documented flags and options for `wc`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for wc>"}, {"role": "assistant", "content": "<correct wc command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<wc with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use wc to...")
