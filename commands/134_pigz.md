# 134 — `pigz` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pigz`** command in ChatML JSONL format.
...
-d -k -r -p -1 -9 -f -c -v --decompress --keep --recursive --processes --fast --best --force --stdout

Cover ALL documented flags and options for `pigz`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pigz>"}, {"role": "assistant", "content": "<correct pigz command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pigz with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pigz to...")
