# 179 — `tcpdump` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`tcpdump`** command in ChatML JSONL format.
...
-i -n -c -w -r -v -vv -vvv -A -X -s -e --interface --numeric --count --write --read --verbose --ascii --hex --snaplen --show-link port host src dst and or not tcp udp icmp

Cover ALL documented flags and options for `tcpdump`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for tcpdump>"}, {"role": "assistant", "content": "<correct tcpdump command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<tcpdump with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use tcpdump to...")
