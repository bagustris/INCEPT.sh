# 191 — `dnf` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`dnf`** command in ChatML JSONL format.
...
install remove update upgrade search info list provides repolist clean history group autoremove makecache --enablerepo --disablerepo -y --best --allowerasing --skip-broken --downloadonly

Cover ALL documented flags and options for `dnf`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dnf>"}, {"role": "assistant", "content": "<correct dnf command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dnf with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dnf to...")
