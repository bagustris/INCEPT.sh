# 159 — `watch` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`watch`** command in ChatML JSONL format.
...
-n -d -g -t -p -e -c --interval --differences --chgexit --no-title --precise --errexit --color

Cover ALL documented flags and options for `watch`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for watch>"}, {"role": "assistant", "content": "<correct watch command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<watch with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use watch to...")
