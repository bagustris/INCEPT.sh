# 085 — `sudo` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`sudo`** command in ChatML JSONL format.
...
-u -i -s -l -k -v -H -E -n --user --login --shell --list --reset-timestamp --validate --set-home --preserve-env --non-interactive

Cover ALL documented flags and options for `sudo`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sudo>"}, {"role": "assistant", "content": "<correct sudo command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sudo with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sudo to...")
