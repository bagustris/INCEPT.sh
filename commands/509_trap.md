# 509 — `trap` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`trap`** command in ChatML JSONL format.
...
EXIT INT TERM HUP ERR DEBUG RETURN

Cover ALL documented flags and options for `trap`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for trap>"}, {"role": "assistant", "content": "<correct trap command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<trap with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use trap to...")
