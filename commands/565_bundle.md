# 565 — `bundle` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`bundle`** command in ChatML JSONL format.

## Command Overview

`bundle` — Ruby Bundler
**Category:** Ruby and Web Frameworks
...
install update exec add remove outdated clean config show --path --without --deployment --jobs

Cover ALL documented flags and options for `bundle`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for bundle>"}, {"role": "assistant", "content": "<correct bundle command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<bundle with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use bundle to...")
