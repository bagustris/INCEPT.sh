# 475 — `rollup` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`rollup`** command in ChatML JSONL format.
...
-c -i -o -f -w -p --config --input --output --format --watch --plugin

Cover ALL documented flags and options for `rollup`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for rollup>"}, {"role": "assistant", "content": "<correct rollup command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<rollup with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use rollup to...")
