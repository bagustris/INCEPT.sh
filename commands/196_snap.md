# 196 — `snap` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`snap`** command in ChatML JSONL format.
...
install remove refresh list find info run connect disconnect alias services start stop restart logs set get

Cover ALL documented flags and options for `snap`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for snap>"}, {"role": "assistant", "content": "<correct snap command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<snap with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use snap to...")
