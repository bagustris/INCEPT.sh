# 398 — `cpupower` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`cpupower`** command in ChatML JSONL format.
...
frequency-info frequency-set idle-info idle-set monitor --governor --min --max

Cover ALL documented flags and options for `cpupower`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cpupower>"}, {"role": "assistant", "content": "<correct cpupower command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cpupower with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cpupower to...")
