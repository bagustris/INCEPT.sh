# 176 — `arp` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`arp`** command in ChatML JSONL format.
...
-a -d -s -n -v --all --delete --set --numeric --verbose

Cover ALL documented flags and options for `arp`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for arp>"}, {"role": "assistant", "content": "<correct arp command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<arp with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use arp to...")
