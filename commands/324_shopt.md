# 324 — `shopt` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`shopt`** command in ChatML JSONL format.
...
-s -u -p -q --set --unset --print --quiet

Cover ALL documented flags and options for `shopt`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for shopt>"}, {"role": "assistant", "content": "<correct shopt command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<shopt with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use shopt to...")
