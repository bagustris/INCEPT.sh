# 231 — `launchctl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`launchctl`** command in ChatML JSONL format.

## Command Overview

`launchctl` — Control macOS launchd
...
load unload start stop list print enable disable bootstrap bootout kickstart blame

Cover ALL documented flags and options for `launchctl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for launchctl>"}, {"role": "assistant", "content": "<correct launchctl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<launchctl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use launchctl to...")
