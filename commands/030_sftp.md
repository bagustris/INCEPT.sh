# 030 — `sftp` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`sftp`** command in ChatML JSONL format.
...
-P -i -b -r -v --port --identity --batchfile --recursive --verbose get put ls cd mkdir rm rmdir

Cover ALL documented flags and options for `sftp`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sftp>"}, {"role": "assistant", "content": "<correct sftp command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sftp with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sftp to...")
