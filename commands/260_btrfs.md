# 260 — `btrfs` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`btrfs`** command in ChatML JSONL format.
...
subvolume filesystem balance device scrub quota check rescue inspect-internal property send receive snapshot create delete list show start stop

Cover ALL documented flags and options for `btrfs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for btrfs>"}, {"role": "assistant", "content": "<correct btrfs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<btrfs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use btrfs to...")
