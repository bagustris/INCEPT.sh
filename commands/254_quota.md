# 254 — `quota` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`quota`** command in ChatML JSONL format.
...
-u -g -v -s --user --group --verbose --human-readable

Cover ALL documented flags and options for `quota`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for quota>"}, {"role": "assistant", "content": "<correct quota command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<quota with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use quota to...")
