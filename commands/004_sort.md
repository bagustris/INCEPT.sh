# 004 — `sort` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`sort`** command in ChatML JSONL format.
...
-n -r -k -t -u -f -h -R -o -s --numeric --reverse --key --field-separator --unique --human-numeric --random --stable --output

Cover ALL documented flags and options for `sort`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sort>"}, {"role": "assistant", "content": "<correct sort command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sort with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sort to...")
