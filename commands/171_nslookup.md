# 171 — `nslookup` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nslookup`** command in ChatML JSONL format.
...
-type= -debug -timeout= -retry= server set type= A AAAA MX NS SOA CNAME TXT PTR

Cover ALL documented flags and options for `nslookup`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nslookup>"}, {"role": "assistant", "content": "<correct nslookup command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nslookup with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nslookup to...")
