# 182 — `iwconfig` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`iwconfig`** command in ChatML JSONL format.
...
essid mode channel freq key rate power txpower retry

Cover ALL documented flags and options for `iwconfig`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for iwconfig>"}, {"role": "assistant", "content": "<correct iwconfig command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<iwconfig with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use iwconfig to...")
