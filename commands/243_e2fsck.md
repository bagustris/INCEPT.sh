# 243 — `e2fsck` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`e2fsck`** command in ChatML JSONL format.

## Command Overview

`e2fsck` — Check ext2/3/4 filesystem
...
-f -y -n -p -v -c --force --yes --no --preen --verbose --check-blocks

Cover ALL documented flags and options for `e2fsck`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for e2fsck>"}, {"role": "assistant", "content": "<correct e2fsck command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<e2fsck with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use e2fsck to...")
