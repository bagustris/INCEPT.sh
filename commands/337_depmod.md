# 337 — `depmod` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`depmod`** command in ChatML JSONL format.

## Command Overview

`depmod` — Generate modules dependency list
...
-a -b -n -v --all --basedir --dry-run --verbose

Cover ALL documented flags and options for `depmod`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for depmod>"}, {"role": "assistant", "content": "<correct depmod command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<depmod with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use depmod to...")
