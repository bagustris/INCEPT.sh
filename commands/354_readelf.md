# 354 — `readelf` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`readelf`** command in ChatML JSONL format.
...
-a -h -l -S -s -d -r -n -V -A --all --file-header --program-headers --section-headers --symbols --dynamic --relocs --notes --version-info --arch-specific

Cover ALL documented flags and options for `readelf`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for readelf>"}, {"role": "assistant", "content": "<correct readelf command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<readelf with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use readelf to...")
