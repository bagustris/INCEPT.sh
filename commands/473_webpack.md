# 473 — `webpack` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`webpack`** command in ChatML JSONL format.
...
--mode --config --env --progress --watch --devtool --output-path --entry development production

Cover ALL documented flags and options for `webpack`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for webpack>"}, {"role": "assistant", "content": "<correct webpack command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<webpack with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use webpack to...")
