# 548 — `hbase-daemon.sh` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`hbase-daemon.sh`** command in ChatML JSONL format.

## Command Overview

`hbase-daemon.sh` — HBase daemon
...
start stop status master regionserver

Cover ALL documented flags and options for `hbase-daemon.sh`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for hbase-daemon.sh>"}, {"role": "assistant", "content": "<correct hbase-daemon.sh command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<hbase-daemon.sh with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use hbase-daemon.sh to...")
