# 399 — `numactl` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`numactl`** command in ChatML JSONL format.
...
--hardware --show --membind --cpunodebind --interleave --physcpubind --preferred

Cover ALL documented flags and options for `numactl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for numactl>"}, {"role": "assistant", "content": "<correct numactl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<numactl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use numactl to...")
