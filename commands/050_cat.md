# 050 — `cat` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`cat`** command in ChatML JSONL format.
...
-n -b -s -A -E -T -v --number --squeeze-blank

Cover ALL documented flags and options for `cat`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cat>"}, {"role": "assistant", "content": "<correct cat command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cat with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cat to...")
