# 372 — `truncate` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`truncate`** command in ChatML JSONL format.
...
-s -c -r --size --no-create --reference

Cover ALL documented flags and options for `truncate`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for truncate>"}, {"role": "assistant", "content": "<correct truncate command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<truncate with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use truncate to...")
