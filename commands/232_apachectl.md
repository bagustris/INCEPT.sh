# 232 — `apachectl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`apachectl`** command in ChatML JSONL format.

## Command Overview

`apachectl` — Apache HTTP server control
...
Cover ALL documented flags and options for `apachectl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for apachectl>"}, {"role": "assistant", "content": "<correct apachectl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<apachectl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use apachectl to...")
