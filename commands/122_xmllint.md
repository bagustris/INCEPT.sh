# 122 — `xmllint` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`xmllint`** command in ChatML JSONL format.

## Command Overview

`xmllint` — XML linter and formatter
...
--format --noout --valid --schema --relaxng --xpath --html --c14n --encode

Cover ALL documented flags and options for `xmllint`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for xmllint>"}, {"role": "assistant", "content": "<correct xmllint command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<xmllint with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use xmllint to...")
