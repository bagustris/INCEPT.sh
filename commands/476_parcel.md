# 476 — `parcel` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`parcel`** command in ChatML JSONL format.
...
build serve watch --port --host --dist-dir --no-cache --no-source-maps

Cover ALL documented flags and options for `parcel`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for parcel>"}, {"role": "assistant", "content": "<correct parcel command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<parcel with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use parcel to...")
