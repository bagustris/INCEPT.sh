# 177 — `route` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`route`** command in ChatML JSONL format.
...
-n -v add del default gw metric dev

Cover ALL documented flags and options for `route`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for route>"}, {"role": "assistant", "content": "<correct route command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<route with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use route to...")
