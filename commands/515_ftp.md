# 515 — `ftp` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`ftp`** command in ChatML JSONL format.
...
open close get put mget mput ls cd lcd binary ascii prompt hash passive

Cover ALL documented flags and options for `ftp`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ftp>"}, {"role": "assistant", "content": "<correct ftp command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ftp with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ftp to...")
