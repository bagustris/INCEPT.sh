# 016 — `lscpu` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`lscpu`** command in ChatML JSONL format.
...
-e -p -J -a -b --extended --parse --json --all --online --offline

Cover ALL documented flags and options for `lscpu`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lscpu>"}, {"role": "assistant", "content": "<correct lscpu command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lscpu with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lscpu to...")
