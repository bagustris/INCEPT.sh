# 405 — `atop` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`atop`** command in ChatML JSONL format.
...
-a -d -m -s -c -g -l -w -r --all --disk --memory --scheduling --various --generic --logging --write --read

Cover ALL documented flags and options for `atop`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for atop>"}, {"role": "assistant", "content": "<correct atop command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<atop with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use atop to...")
