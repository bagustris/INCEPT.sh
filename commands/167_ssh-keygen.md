# 167 — `ssh-keygen` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ssh-keygen`** command in ChatML JSONL format.
...
-t -b -f -C -N -p -l -R -y --type --bits --filename --comment --new-passphrase --change-passphrase --fingerprint --remove --pubkey rsa ed25519 ecdsa

Cover ALL documented flags and options for `ssh-keygen`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ssh-keygen>"}, {"role": "assistant", "content": "<correct ssh-keygen command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ssh-keygen with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ssh-keygen to...")
