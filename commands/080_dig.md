# 080 — `dig` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`dig`** command in ChatML JSONL format.
...
+short +trace +noall +answer +stats -t -x -p @server ANY A AAAA MX NS SOA CNAME TXT PTR SRV

Cover ALL documented flags and options for `dig`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dig>"}, {"role": "assistant", "content": "<correct dig command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dig with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dig to...")
