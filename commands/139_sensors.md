# 139 — `sensors` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`sensors`** command in ChatML JSONL format.

## Command Overview

`sensors` — Print hardware sensors data
...
-f -A -u -j --fahrenheit --no-adapter --raw --json

Cover ALL documented flags and options for `sensors`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sensors>"}, {"role": "assistant", "content": "<correct sensors command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sensors with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sensors to...")
