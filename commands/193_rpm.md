# 193 — `rpm` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`rpm`** command in ChatML JSONL format.
...
-i -U -e -q -V -K -qa -qi -ql -qf -qR --install --upgrade --erase --query --verify --checksig --all --info --list --file --requires --whatprovides --import

Cover ALL documented flags and options for `rpm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for rpm>"}, {"role": "assistant", "content": "<correct rpm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<rpm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use rpm to...")
