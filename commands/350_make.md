# 350 — `make` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`make`** command in ChatML JSONL format.
...
-f -j -k -n -B -C -d -s --file --jobs --keep-going --dry-run --always-make --directory --debug --silent clean all install

Cover ALL documented flags and options for `make`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for make>"}, {"role": "assistant", "content": "<correct make command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<make with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use make to...")
