# 007 — `hexdump` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`hexdump`** command in ChatML JSONL format.

## Command Overview

`hexdump` — ASCII octal decimal hex dump
...
-C -b -c -d -o -x -n -s -v --canonical

Cover ALL documented flags and options for `hexdump`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for hexdump>"}, {"role": "assistant", "content": "<correct hexdump command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<hexdump with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use hexdump to...")
