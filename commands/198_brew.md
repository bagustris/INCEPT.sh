# 198 — `brew` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`brew`** command in ChatML JSONL format.
...
install uninstall upgrade update list search info outdated cleanup doctor deps autoremove services tap untap --cask --formula --verbose --force

Cover ALL documented flags and options for `brew`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for brew>"}, {"role": "assistant", "content": "<correct brew command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<brew with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use brew to...")
