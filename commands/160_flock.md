# 160 — `flock` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`flock`** command in ChatML JSONL format.
...
-s -x -u -n -w -o --shared --exclusive --unlock --nonblock --wait --close

Cover ALL documented flags and options for `flock`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for flock>"}, {"role": "assistant", "content": "<correct flock command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<flock with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use flock to...")
