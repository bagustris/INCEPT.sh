# 123 — `gzip` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`gzip`** command in ChatML JSONL format.
...
-d -k -r -v -1 -9 -f -c -l -n -N --decompress --keep --recursive --verbose --fast --best --force --stdout --list

Cover ALL documented flags and options for `gzip`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for gzip>"}, {"role": "assistant", "content": "<correct gzip command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<gzip with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use gzip to...")
