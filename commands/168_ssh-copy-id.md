# 168 — `ssh-copy-id` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ssh-copy-id`** command in ChatML JSONL format.
...
-i -p -f --identity --port --force

Cover ALL documented flags and options for `ssh-copy-id`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ssh-copy-id>"}, {"role": "assistant", "content": "<correct ssh-copy-id command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ssh-copy-id with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ssh-copy-id to...")
