# 135 — `hostnamectl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`hostnamectl`** command in ChatML JSONL format.
...
status set-hostname set-icon-name set-chassis set-deployment --static --transient --pretty

Cover ALL documented flags and options for `hostnamectl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for hostnamectl>"}, {"role": "assistant", "content": "<correct hostnamectl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<hostnamectl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use hostnamectl to...")
