# 245 — `resize2fs` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`resize2fs`** command in ChatML JSONL format.
...
-f -p -M --force --progress --minimum

Cover ALL documented flags and options for `resize2fs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for resize2fs>"}, {"role": "assistant", "content": "<correct resize2fs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<resize2fs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use resize2fs to...")
