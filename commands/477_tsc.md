# 477 — `tsc` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`tsc`** command in ChatML JSONL format.
...
--init --watch --build --project --target --module --outDir --strict --noEmit --declaration --sourceMap

Cover ALL documented flags and options for `tsc`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for tsc>"}, {"role": "assistant", "content": "<correct tsc command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<tsc with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use tsc to...")
