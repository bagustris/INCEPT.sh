# 144 — `cal` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`cal`** command in ChatML JSONL format.
...
-y -3 -m -j -s -n --year --three --monday --julian --sunday --months

Cover ALL documented flags and options for `cal`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cal>"}, {"role": "assistant", "content": "<correct cal command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cal with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cal to...")
