# 109 — `xargs` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`xargs`** command in ChatML JSONL format.
...
-0 -I -n -P -d -t -r --null --replace --max-args --max-procs --delimiter --verbose --no-run-if-empty

Cover ALL documented flags and options for `xargs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for xargs>"}, {"role": "assistant", "content": "<correct xargs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<xargs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use xargs to...")
