# 020 — `top` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`top`** command in ChatML JSONL format.
...
-b -n -d -p -u -o -H -c --batch --iterations --delay --pid --user --sort --threads --command-line

Cover ALL documented flags and options for `top`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for top>"}, {"role": "assistant", "content": "<correct top command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<top with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use top to...")
