# 451 — `squid` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`squid`** command in ChatML JSONL format.
...
-N -d -f -k -z --no-daemon --debug --config --reconfigure --shutdown

Cover ALL documented flags and options for `squid`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for squid>"}, {"role": "assistant", "content": "<correct squid command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<squid with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use squid to...")
