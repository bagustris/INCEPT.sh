# 075 — `ping` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`ping`** command in ChatML JSONL format.
...
-c -i -s -t -W -w -4 -6 -q -f -n -I --count --interval --packetsize --ttl --timeout --deadline --quiet --flood --numeric --interface

Cover ALL documented flags and options for `ping`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ping>"}, {"role": "assistant", "content": "<correct ping command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ping with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ping to...")
