# 189 — `apt-mark` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`apt-mark`** command in ChatML JSONL format.

## Command Overview

`apt-mark` — Mark/unmark packages as manually installed
...
auto manual hold unhold showmanual showauto showhold

Cover ALL documented flags and options for `apt-mark`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for apt-mark>"}, {"role": "assistant", "content": "<correct apt-mark command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<apt-mark with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use apt-mark to...")
