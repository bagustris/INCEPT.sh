# 128 — `zstd` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`zstd`** command in ChatML JSONL format.
...
-d -k -r -v -1 -19 --ultra -f -c -o -T --decompress --keep --recursive --verbose --force --stdout --threads

Cover ALL documented flags and options for `zstd`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for zstd>"}, {"role": "assistant", "content": "<correct zstd command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<zstd with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use zstd to...")
