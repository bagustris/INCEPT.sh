# 137 — `w` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`w`** command in ChatML JSONL format.
...
-h -s -f -i --no-header --short --from --ip-addr

Cover ALL documented flags and options for `w`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for w>"}, {"role": "assistant", "content": "<correct w command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<w with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use w to...")
