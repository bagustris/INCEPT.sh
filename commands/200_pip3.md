# 200 — `pip3` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pip3`** command in ChatML JSONL format.
...
install uninstall list show freeze download -r --requirement -U --upgrade --user --no-deps --force-reinstall

Cover ALL documented flags and options for `pip3`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pip3>"}, {"role": "assistant", "content": "<correct pip3 command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pip3 with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pip3 to...")
