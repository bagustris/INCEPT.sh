# 066 — `date` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`date`** command in ChatML JSONL format.
...
-u -d -s -r --utc --date --set --reference +FORMAT %Y %m %d %H %M %S %F %T %s %A %B %Z

Cover ALL documented flags and options for `date`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for date>"}, {"role": "assistant", "content": "<correct date command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<date with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use date to...")
