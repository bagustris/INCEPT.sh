# 353 — `valgrind` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`valgrind`** command in ChatML JSONL format.
...
--leak-check=full --show-reachable --track-origins --tool= --log-file= --gen-suppressions= memcheck callgrind cachegrind massif helgrind

Cover ALL documented flags and options for `valgrind`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for valgrind>"}, {"role": "assistant", "content": "<correct valgrind command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<valgrind with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use valgrind to...")
