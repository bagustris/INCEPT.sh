# 333 — `modprobe` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`modprobe`** command in ChatML JSONL format.
...
-r -v -n -f -a --remove --verbose --dry-run --force --all

Cover ALL documented flags and options for `modprobe`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for modprobe>"}, {"role": "assistant", "content": "<correct modprobe command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<modprobe with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use modprobe to...")
