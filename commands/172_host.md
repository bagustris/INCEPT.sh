# 172 — `host` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`host`** command in ChatML JSONL format.
...
-t -a -l -v -T -4 -6 --type --all --list --verbose

Cover ALL documented flags and options for `host`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for host>"}, {"role": "assistant", "content": "<correct host command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<host with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use host to...")
