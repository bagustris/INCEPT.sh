# 037 — `chmod` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`chmod`** command in ChatML JSONL format.
...
- `--reference`: Set permissions to match the specified reference file instead of a mode value

## Additional Flags to Cover

777 755 700 644 600 400 +x -x +r -r +w -w u+x g+w o-r a+rx u=rwx -R --recursive --verbose --changes --reference

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for chmod>"}, {"role": "assistant", "content": "<correct chmod command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<chmod with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use chmod to...")
