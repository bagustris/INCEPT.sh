# 165 — `ulimit` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ulimit`** command in ChatML JSONL format.
...
-a -c -d -f -l -m -n -s -t -u -v -H -S --all --core --data --fsize --locks --memlock --nofile --nproc --stack --cpu

Cover ALL documented flags and options for `ulimit`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ulimit>"}, {"role": "assistant", "content": "<correct ulimit command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ulimit with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ulimit to...")
