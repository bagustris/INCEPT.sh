# 279 — `ufw` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ufw`** command in ChatML JSONL format.
...
enable disable status allow deny reject limit delete route reset default logging app --dry-run --force

Cover ALL documented flags and options for `ufw`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ufw>"}, {"role": "assistant", "content": "<correct ufw command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ufw with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ufw to...")
