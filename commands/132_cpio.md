# 132 — `cpio` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`cpio`** command in ChatML JSONL format.
...
-i -o -p -d -v -t -F --extract --create --pass-through --make-directories --verbose --list --file

Cover ALL documented flags and options for `cpio`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cpio>"}, {"role": "assistant", "content": "<correct cpio command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cpio with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cpio to...")
