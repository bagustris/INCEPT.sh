# 199 — `pip` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pip`** command in ChatML JSONL format.
...
install uninstall list show freeze search download check -r --requirement -U --upgrade --user --target --index-url --no-deps --force-reinstall --no-cache-dir -e --editable

Cover ALL documented flags and options for `pip`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pip>"}, {"role": "assistant", "content": "<correct pip command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pip with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pip to...")
