# 011 — `nano` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`nano`** command in ChatML JSONL format.
...
-B -C -i -l -m -w --backup --backupdir --autoindent --linenumbers --mouse --nowrap

Cover ALL documented flags and options for `nano`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nano>"}, {"role": "assistant", "content": "<correct nano command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nano with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nano to...")
