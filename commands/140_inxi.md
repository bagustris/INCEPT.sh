# 140 — `inxi` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`inxi`** command in ChatML JSONL format.

## Command Overview

`inxi` — Full featured system info script
...
-F -b -c -G -A -N -i -o -p -r -S -t --full --basic --cpu --graphics --audio --network --ip --repos --partitions --sensors

Cover ALL documented flags and options for `inxi`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for inxi>"}, {"role": "assistant", "content": "<correct inxi command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<inxi with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use inxi to...")
