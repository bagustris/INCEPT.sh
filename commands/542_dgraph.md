# 542 — `dgraph` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`dgraph`** command in ChatML JSONL format.

## Command Overview

`dgraph` — Dgraph database
...
alpha zero bulk live tool --lru_mb --raft --my --peer

Cover ALL documented flags and options for `dgraph`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dgraph>"}, {"role": "assistant", "content": "<correct dgraph command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dgraph with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dgraph to...")
