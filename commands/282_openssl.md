# 282 — `openssl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`openssl`** command in ChatML JSONL format.
...
genrsa genpkey req x509 s_client s_server enc dgst rand verify pkcs12 rsa ec pkey crl ca passwd prime -in -out -key -cert -CAfile -CApath -subj -days -nodes -newkey -new -sha256 -aes256 -text -noout -connect

Cover ALL documented flags and options for `openssl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for openssl>"}, {"role": "assistant", "content": "<correct openssl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<openssl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use openssl to...")
