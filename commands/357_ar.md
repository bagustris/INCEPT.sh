# 357 — `ar` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ar`** command in ChatML JSONL format.
...
r c s d t x q --create --extract --list --delete --replace --quick-append

Cover ALL documented flags and options for `ar`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ar>"}, {"role": "assistant", "content": "<correct ar command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ar with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ar to...")
