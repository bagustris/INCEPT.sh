# 461 — `elixir` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`elixir`** command in ChatML JSONL format.
...
-e -r -S -pa --eval --require --script

Cover ALL documented flags and options for `elixir`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for elixir>"}, {"role": "assistant", "content": "<correct elixir command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<elixir with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use elixir to...")
