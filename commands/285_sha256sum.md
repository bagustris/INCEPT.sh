# 285 — `sha256sum` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`sha256sum`** command in ChatML JSONL format.
...
Cover ALL documented flags and options for `sha256sum`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sha256sum>"}, {"role": "assistant", "content": "<correct sha256sum command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sha256sum with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sha256sum to...")
