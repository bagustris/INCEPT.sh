# 380 — `namei` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`namei`** command in ChatML JSONL format.

## Command Overview

`namei` — Follow pathname to terminal point
...
-l -m -x --long --modes --mountpoints

Cover ALL documented flags and options for `namei`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for namei>"}, {"role": "assistant", "content": "<correct namei command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<namei with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use namei to...")
