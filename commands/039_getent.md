# 039 — `getent` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`getent`** command in ChatML JSONL format.
...
passwd group hosts services protocols networks shadow

Cover ALL documented flags and options for `getent`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for getent>"}, {"role": "assistant", "content": "<correct getent command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<getent with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use getent to...")
