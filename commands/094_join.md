# 094 — `join` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`join`** command in ChatML JSONL format.
...
-1 -2 -t -i -a -e -o --check-order

Cover ALL documented flags and options for `join`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for join>"}, {"role": "assistant", "content": "<correct join command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<join with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use join to...")
