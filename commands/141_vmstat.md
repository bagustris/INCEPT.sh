# 141 — `vmstat` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`vmstat`** command in ChatML JSONL format.
...
-a -d -D -f -m -n -p -s -S -t -w --active --disk --disk-sum --forks --slabs --stats --timestamp --wide

Cover ALL documented flags and options for `vmstat`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for vmstat>"}, {"role": "assistant", "content": "<correct vmstat command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<vmstat with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use vmstat to...")
