# 100 — `tr` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`tr`** command in ChatML JSONL format.
...
-d -s -c --delete --squeeze-repeats --complement [:upper:] [:lower:] [:digit:] [:alpha:] [:space:]

Cover ALL documented flags and options for `tr`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for tr>"}, {"role": "assistant", "content": "<correct tr command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<tr with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use tr to...")
