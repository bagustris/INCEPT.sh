# 281 — `pfctl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pfctl`** command in ChatML JSONL format.

## Command Overview

`pfctl` — PF packet filter control (macOS/BSD)
...
-e -d -f -F -s -t -T -v --enable --disable --flush --show --table

Cover ALL documented flags and options for `pfctl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pfctl>"}, {"role": "assistant", "content": "<correct pfctl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pfctl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pfctl to...")
