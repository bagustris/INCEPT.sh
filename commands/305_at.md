# 305 — `at` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`at`** command in ChatML JSONL format.

## Command Overview

`at` — Execute commands at scheduled time
**Category:** Scheduling
...
-f -l -d -c -m -q -v --file --list --delete --cat --mail --queue --verbose now noon midnight tomorrow teatime

Cover ALL documented flags and options for `at`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for at>"}, {"role": "assistant", "content": "<correct at command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<at with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use at to...")
