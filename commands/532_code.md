# 532 — `code` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`code`** command in ChatML JSONL format.
...
-r -g -d -n --reuse-window --goto --diff --new-window --install-extension --uninstall-extension --list-extensions

Cover ALL documented flags and options for `code`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for code>"}, {"role": "assistant", "content": "<correct code command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<code with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use code to...")
