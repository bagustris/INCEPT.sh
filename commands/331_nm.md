# 331 — `nm` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nm`** command in ChatML JSONL format.
...
-g -u -D -C -A -n --extern-only --undefined-only --dynamic --demangle --print-file-name --numeric-sort

Cover ALL documented flags and options for `nm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nm>"}, {"role": "assistant", "content": "<correct nm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nm to...")
