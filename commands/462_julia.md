# 462 — `julia` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`julia`** command in ChatML JSONL format.
...
-e -p -t --eval --procs --threads --project --startup-file

Cover ALL documented flags and options for `julia`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for julia>"}, {"role": "assistant", "content": "<correct julia command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<julia with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use julia to...")
