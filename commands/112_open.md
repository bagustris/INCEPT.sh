# 112 — `open` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`open`** command in ChatML JSONL format.

## Command Overview

`open` — Open files/URLs with default app (macOS)
...
Cover ALL documented flags and options for `open`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for open>"}, {"role": "assistant", "content": "<correct open command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<open with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use open to...")
