# 108 — `column` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`column`** command in ChatML JSONL format.
...
-t -s -n -c -o --table --separator --output-separator

Cover ALL documented flags and options for `column`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for column>"}, {"role": "assistant", "content": "<correct column command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<column with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use column to...")
