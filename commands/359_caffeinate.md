# 359 — `caffeinate` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`caffeinate`** command in ChatML JSONL format.
...
-d -i -m -s -u -t --display --idle --disk --system --user --timeout

Cover ALL documented flags and options for `caffeinate`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for caffeinate>"}, {"role": "assistant", "content": "<correct caffeinate command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<caffeinate with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use caffeinate to...")
