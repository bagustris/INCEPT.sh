# 570 — `paccache` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`paccache`** command in ChatML JSONL format.
...
-r -d -u -v --remove --dryrun --uninstalled --verbose

Cover ALL documented flags and options for `paccache`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for paccache>"}, {"role": "assistant", "content": "<correct paccache command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<paccache with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use paccache to...")
