# 074 — `lsof` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`lsof`** command in ChatML JSONL format.
...
-i -p -u -c -t -n -P +D -d --internet --pid --user --command --terse

Cover ALL documented flags and options for `lsof`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lsof>"}, {"role": "assistant", "content": "<correct lsof command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lsof with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lsof to...")
