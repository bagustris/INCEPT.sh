# 013 — `vim` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`vim`** command in ChatML JSONL format.
...
-R -c -o -O -p -d -u -N +line +:command --readonly --noplugin

Cover ALL documented flags and options for `vim`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for vim>"}, {"role": "assistant", "content": "<correct vim command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<vim with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use vim to...")
