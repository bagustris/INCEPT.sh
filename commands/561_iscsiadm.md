# 561 — `iscsiadm` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`iscsiadm`** command in ChatML JSONL format.

## Command Overview

`iscsiadm` — iSCSI administration
**Category:** Miscellaneous Servers
...
-m --mode node session discovery interface host --type --targetname --portal --login --logout --rescan

Cover ALL documented flags and options for `iscsiadm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for iscsiadm>"}, {"role": "assistant", "content": "<correct iscsiadm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<iscsiadm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use iscsiadm to...")
