# 446 — `influx` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`influx`** command in ChatML JSONL format.
...
-host -port -username -password -database -execute -format -precision write query

Cover ALL documented flags and options for `influx`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for influx>"}, {"role": "assistant", "content": "<correct influx command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<influx with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use influx to...")
