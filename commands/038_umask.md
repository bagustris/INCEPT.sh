# 038 — `umask` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`umask`** command in ChatML JSONL format.
...
022 077 027 002 000 -S --symbolic

Cover ALL documented flags and options for `umask`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for umask>"}, {"role": "assistant", "content": "<correct umask command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<umask with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use umask to...")
