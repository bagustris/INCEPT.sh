# 519 — `inotifywait` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`inotifywait`** command in ChatML JSONL format.
...
-m -r -e -t -o --monitor --recursive --event --timeout --outfile create modify delete move access

Cover ALL documented flags and options for `inotifywait`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for inotifywait>"}, {"role": "assistant", "content": "<correct inotifywait command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<inotifywait with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use inotifywait to...")
