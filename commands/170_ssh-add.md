# 170 — `ssh-add` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ssh-add`** command in ChatML JSONL format.
...
-l -L -d -D -t -K --list --delete --delete-all --lifetime

Cover ALL documented flags and options for `ssh-add`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ssh-add>"}, {"role": "assistant", "content": "<correct ssh-add command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ssh-add with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ssh-add to...")
