# 342 — `dracut` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`dracut`** command in ChatML JSONL format.

## Command Overview

`dracut` — Create initial ramdisk
...
-f -v -k --force --verbose --kver --add --omit --install

Cover ALL documented flags and options for `dracut`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dracut>"}, {"role": "assistant", "content": "<correct dracut command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dracut with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dracut to...")
