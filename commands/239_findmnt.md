# 239 — `findmnt` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`findmnt`** command in ChatML JSONL format.
...
-t -o -n -l -r -J -T --type --output --noheadings --list --raw --json --target --source

Cover ALL documented flags and options for `findmnt`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for findmnt>"}, {"role": "assistant", "content": "<correct findmnt command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<findmnt with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use findmnt to...")
