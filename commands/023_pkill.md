# 023 — `pkill` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`pkill`** command in ChatML JSONL format.
...
-f -u -t -P -g -x -n -o --full --uid --terminal --parent --pgroup --exact --newest --oldest

Cover ALL documented flags and options for `pkill`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pkill>"}, {"role": "assistant", "content": "<correct pkill command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pkill with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pkill to...")
