# 526 — `pactl` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`pactl`** command in ChatML JSONL format.

## Command Overview

`pactl` — PulseAudio control
**Category:** Miscellaneous CLI Tools
...
list info set-sink-volume set-source-volume set-sink-mute set-source-mute get-sink-volume set-default-sink set-default-source

Cover ALL documented flags and options for `pactl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pactl>"}, {"role": "assistant", "content": "<correct pactl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pactl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pactl to...")
