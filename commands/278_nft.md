# 278 — `nft` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nft`** command in ChatML JSONL format.

## Command Overview

`nft` — Nftables packet filtering framework
...
add delete list flush create rename table chain rule set map counter quota limit monitor export import

Cover ALL documented flags and options for `nft`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nft>"}, {"role": "assistant", "content": "<correct nft command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nft with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nft to...")
