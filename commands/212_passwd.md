# 212 — `passwd` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`passwd`** command in ChatML JSONL format.
...
-l -u -d -e -n -x -w -i -S --lock --unlock --delete --expire --mindays --maxdays --warndays --inactive --status

Cover ALL documented flags and options for `passwd`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for passwd>"}, {"role": "assistant", "content": "<correct passwd command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<passwd with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use passwd to...")
