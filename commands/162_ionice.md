# 162 — `ionice` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ionice`** command in ChatML JSONL format.
...
-c -n -p -t --class --classdata --pid --ignore

Cover ALL documented flags and options for `ionice`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ionice>"}, {"role": "assistant", "content": "<correct ionice command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ionice with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ionice to...")
