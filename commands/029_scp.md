# 029 — `scp` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`scp`** command in ChatML JSONL format.
...
- `-P`: Specify the port to connect to on the remote host (uppercase P, unlike ssh)
- `-r`: Recursively copy entire directories
- `-p`: Preserve modification times, access times, and modes from the original files
- `-C`: Enable compression during transfer
- `-i`: Select the identity file (private key) for public key authentication

## Additional Flags to Cover

-r -P -i -v -C -p -q -l --recursive --port --identity --verbose --compress --preserve --quiet --limit

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for scp>"}, {"role": "assistant", "content": "<correct scp command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<scp with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use scp to...")
