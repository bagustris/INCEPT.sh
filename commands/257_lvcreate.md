# 257 — `lvcreate` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`lvcreate`** command in ChatML JSONL format.
...
-L -l -n -s -T --size --extents --name --snapshot --thin

Cover ALL documented flags and options for `lvcreate`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lvcreate>"}, {"role": "assistant", "content": "<correct lvcreate command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lvcreate with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lvcreate to...")
