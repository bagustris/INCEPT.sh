# 250 — `fstrim` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`fstrim`** command in ChatML JSONL format.

## Command Overview

`fstrim` — Discard unused filesystem blocks
...
-a -v --all --verbose --minimum --offset --length

Cover ALL documented flags and options for `fstrim`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for fstrim>"}, {"role": "assistant", "content": "<correct fstrim command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<fstrim with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use fstrim to...")
