# 419 — `xfs_growfs` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`xfs_growfs`** command in ChatML JSONL format.
...
-d -D -L -l --data --data-size --log --log-size

Cover ALL documented flags and options for `xfs_growfs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for xfs_growfs>"}, {"role": "assistant", "content": "<correct xfs_growfs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<xfs_growfs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use xfs_growfs to...")
