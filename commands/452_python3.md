# 452 — `python3` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`python3`** command in ChatML JSONL format.
...
-c -m -i -u -v -B -O -W --command --module --interactive --unbuffered --verbose

Cover ALL documented flags and options for `python3`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for python3>"}, {"role": "assistant", "content": "<correct python3 command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<python3 with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use python3 to...")
