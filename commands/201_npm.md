# 201 — `npm` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`npm`** command in ChatML JSONL format.
...
install uninstall update list search init run test build start audit fix pack publish --save --save-dev --global -g --production --legacy-peer-deps ci

Cover ALL documented flags and options for `npm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for npm>"}, {"role": "assistant", "content": "<correct npm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<npm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use npm to...")
