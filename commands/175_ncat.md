# 175 — `ncat` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ncat`** command in ChatML JSONL format.
...
-l -p -u -v -k -e -c --listen --port --udp --verbose --keep-open --exec --sh-exec --ssl

Cover ALL documented flags and options for `ncat`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ncat>"}, {"role": "assistant", "content": "<correct ncat command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ncat with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ncat to...")
