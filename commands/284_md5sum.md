# 284 — `md5sum` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`md5sum`** command in ChatML JSONL format.
...
-c -b -t --check --binary --text --quiet --status --warn

Cover ALL documented flags and options for `md5sum`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for md5sum>"}, {"role": "assistant", "content": "<correct md5sum command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<md5sum with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use md5sum to...")
