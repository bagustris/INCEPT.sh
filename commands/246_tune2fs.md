# 246 — `tune2fs` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`tune2fs`** command in ChatML JSONL format.
...
-l -c -i -m -L -O -e -U --list --max-mount-counts --interval --reserved-blocks-percentage --label --feature --error-behavior --uuid

Cover ALL documented flags and options for `tune2fs`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for tune2fs>"}, {"role": "assistant", "content": "<correct tune2fs command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<tune2fs with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use tune2fs to...")
