# 479 — `eslint` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`eslint`** command in ChatML JSONL format.
...
--fix --config --ext --ignore-path --max-warnings --format --rule --debug

Cover ALL documented flags and options for `eslint`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for eslint>"}, {"role": "assistant", "content": "<correct eslint command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<eslint with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use eslint to...")
