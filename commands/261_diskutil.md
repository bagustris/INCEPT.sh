# 261 — `diskutil` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`diskutil`** command in ChatML JSONL format.

## Command Overview

`diskutil` — Disk utility (macOS)
**Category:** Disk and Filesystem
...
list info mount unmount eject partitionDisk eraseDisk eraseVolume verifyDisk repairDisk apfs addVolume deleteVolume

Cover ALL documented flags and options for `diskutil`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for diskutil>"}, {"role": "assistant", "content": "<correct diskutil command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<diskutil with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use diskutil to...")
