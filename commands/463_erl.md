# 463 — `erl` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`erl`** command in ChatML JSONL format.
...
-sname -name -setcookie -pa -pz -eval -noshell -detached

Cover ALL documented flags and options for `erl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for erl>"}, {"role": "assistant", "content": "<correct erl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<erl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use erl to...")
