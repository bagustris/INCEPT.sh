# 435 — `bash` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`bash`** command in ChatML JSONL format.
...
-c -l -i -r -s -x -v -n --login --interactive --restricted --verbose --norc --noprofile

Cover ALL documented flags and options for `bash`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for bash>"}, {"role": "assistant", "content": "<correct bash command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<bash with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use bash to...")
