# 148 — `system_profiler` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`system_profiler`** command in ChatML JSONL format.

## Command Overview

`system_profiler` — System information (macOS)
...
SPHardwareDataType SPSoftwareDataType SPNetworkDataType SPStorageDataType SPMemoryDataType SPDisplaysDataType SPUSBDataType -json -xml -detailLevel

Cover ALL documented flags and options for `system_profiler`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for system_profiler>"}, {"role": "assistant", "content": "<correct system_profiler command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<system_profiler with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use system_profiler to...")
