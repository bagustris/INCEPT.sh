# 070 — `du` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`du`** command in ChatML JSONL format.
...
- `-d`: Display totals for directories at most N levels deep
- `--max-depth`: Display totals for directories at most N levels deep (GNU long form)
- `--apparent-size`: Print apparent sizes rather than disk usage
- `--exclude`: Exclude files matching the given glob pattern

## Additional Flags to Cover

-s -h -a -c -d --max-depth --summarize --human-readable --all --total --apparent-size --block-size --exclude --time -x

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for du>"}, {"role": "assistant", "content": "<correct du command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<du with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use du to...")
