# 274 — `unshare` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`unshare`** command in ChatML JSONL format.
...
-m -u -i -n -p -U -f --mount --uts --ipc --net --pid --user --fork --map-root-user

Cover ALL documented flags and options for `unshare`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for unshare>"}, {"role": "assistant", "content": "<correct unshare command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<unshare with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use unshare to...")
