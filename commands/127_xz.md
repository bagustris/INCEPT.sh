# 127 — `xz` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`xz`** command in ChatML JSONL format.
...
-d -k -z -v -0 -9 -e -f -c -l -T --decompress --keep --compress --verbose --extreme --force --stdout --list --threads

Cover ALL documented flags and options for `xz`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for xz>"}, {"role": "assistant", "content": "<correct xz command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<xz with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use xz to...")
