# 009 — `lsattr` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`lsattr`** command in ChatML JSONL format.

## Command Overview

`lsattr` — List file attributes on ext filesystem
...
-R -a -d -v --recursive --all --directory

Cover ALL documented flags and options for `lsattr`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lsattr>"}, {"role": "assistant", "content": "<correct lsattr command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lsattr with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lsattr to...")
