# 126 — `bunzip2` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`bunzip2`** command in ChatML JSONL format.
...
-k -v -f -c --keep --verbose --force --stdout

Cover ALL documented flags and options for `bunzip2`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for bunzip2>"}, {"role": "assistant", "content": "<correct bunzip2 command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<bunzip2 with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use bunzip2 to...")
