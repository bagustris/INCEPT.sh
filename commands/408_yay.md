# 408 — `yay` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`yay`** command in ChatML JSONL format.
...
-S -R -Q -Ss -Si -Sua --sync --remove --query --search --info --sysupgrade

Cover ALL documented flags and options for `yay`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for yay>"}, {"role": "assistant", "content": "<correct yay command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<yay with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use yay to...")
