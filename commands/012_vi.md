# 012 — `vi` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`vi`** command in ChatML JSONL format.
...
-R -c -o -O -p +line +:command --readonly

Cover ALL documented flags and options for `vi`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for vi>"}, {"role": "assistant", "content": "<correct vi command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<vi with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use vi to...")
