# 005 — `uniq` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`uniq`** command in ChatML JSONL format.
...
-c -d -u -i -f -s -w --count --repeated --unique --ignore-case

Cover ALL documented flags and options for `uniq`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for uniq>"}, {"role": "assistant", "content": "<correct uniq command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<uniq with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use uniq to...")
