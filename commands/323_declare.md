# 323 — `declare` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`declare`** command in ChatML JSONL format.

## Command Overview

`declare` — Declare variables and attributes
**Category:** Shell Builtins and Utilities
...
-a -A -f -i -l -r -u -x -p --array --assoc --function --integer --lower --readonly --upper --export --print

Cover ALL documented flags and options for `declare`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for declare>"}, {"role": "assistant", "content": "<correct declare command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<declare with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use declare to...")
