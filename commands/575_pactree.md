# 575 — `pactree` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`pactree`** command in ChatML JSONL format.

## Command Overview

`pactree` — Package dependency tree
...
-r -d -s -u --reverse --depth --sync --unique

Cover ALL documented flags and options for `pactree`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pactree>"}, {"role": "assistant", "content": "<correct pactree command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pactree with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pactree to...")
