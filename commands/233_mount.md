# 233 — `mount` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`mount`** command in ChatML JSONL format.
...
-t -o -a -r -w -v -n --type --options --all --read-only --read-write --verbose --no-mtab remount bind loop noexec nosuid nodev

Cover ALL documented flags and options for `mount`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for mount>"}, {"role": "assistant", "content": "<correct mount command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<mount with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use mount to...")
