# 381 — `shuf` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`shuf`** command in ChatML JSONL format.
...
-n -i -e -o -r --head-count --input-range --echo --output --repeat

Cover ALL documented flags and options for `shuf`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for shuf>"}, {"role": "assistant", "content": "<correct shuf command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<shuf with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use shuf to...")
