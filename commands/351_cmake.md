# 351 — `cmake` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`cmake`** command in ChatML JSONL format.
...
-B -S -G -D -DCMAKE_BUILD_TYPE= -DCMAKE_INSTALL_PREFIX= --build --install --target --config --preset --list-presets

Cover ALL documented flags and options for `cmake`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cmake>"}, {"role": "assistant", "content": "<correct cmake command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cmake with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cmake to...")
