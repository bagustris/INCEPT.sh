# 147 — `sw_vers` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`sw_vers`** command in ChatML JSONL format.

## Command Overview

`sw_vers` — Print macOS version info (macOS)
...
Cover ALL documented flags and options for `sw_vers`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sw_vers>"}, {"role": "assistant", "content": "<correct sw_vers command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sw_vers with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sw_vers to...")
