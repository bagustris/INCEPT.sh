# 235 — `fdisk` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`fdisk`** command in ChatML JSONL format.
...
-l -u -b -c -n -d -p -t -w --list --units --sector-size --cylinders

Cover ALL documented flags and options for `fdisk`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for fdisk>"}, {"role": "assistant", "content": "<correct fdisk command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<fdisk with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use fdisk to...")
