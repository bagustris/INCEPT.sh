# 220 — `lastb` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`lastb`** command in ChatML JSONL format.
...
-n -f -a --num --file --hostlast

Cover ALL documented flags and options for `lastb`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lastb>"}, {"role": "assistant", "content": "<correct lastb command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lastb with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lastb to...")
