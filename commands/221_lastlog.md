# 221 — `lastlog` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`lastlog`** command in ChatML JSONL format.

## Command Overview

`lastlog` — Report most recent login of users
...
-b -t -u --before --time --user

Cover ALL documented flags and options for `lastlog`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lastlog>"}, {"role": "assistant", "content": "<correct lastlog command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lastlog with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lastlog to...")
