# 486 — `ng` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`ng`** command in ChatML JSONL format.
...
new generate serve build test lint e2e add update deploy --routing --style --skip-tests --standalone --project

Cover ALL documented flags and options for `ng`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ng>"}, {"role": "assistant", "content": "<correct ng command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ng with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ng to...")
