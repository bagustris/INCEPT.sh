# 208 — `groupadd` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`groupadd`** command in ChatML JSONL format.
...
-g -r -f --gid --system --force

Cover ALL documented flags and options for `groupadd`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for groupadd>"}, {"role": "assistant", "content": "<correct groupadd command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<groupadd with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use groupadd to...")
