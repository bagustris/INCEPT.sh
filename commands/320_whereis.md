# 320 — `whereis` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`whereis`** command in ChatML JSONL format.
...
-b -m -s --binary --manual --source

Cover ALL documented flags and options for `whereis`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for whereis>"}, {"role": "assistant", "content": "<correct whereis command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<whereis with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use whereis to...")
