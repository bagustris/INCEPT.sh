# 058 — `diff` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`diff`** command in ChatML JSONL format.
...
-u -y -r -q -s -N -i -w -B --unified --side-by-side --recursive --brief --color

Cover ALL documented flags and options for `diff`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for diff>"}, {"role": "assistant", "content": "<correct diff command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<diff with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use diff to...")
