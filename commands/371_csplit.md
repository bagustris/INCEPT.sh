# 371 — `csplit` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`csplit`** command in ChatML JSONL format.

## Command Overview

`csplit` — Split file into context-determined pieces
...
-f -n -z -b --prefix --digits --elide-empty-files

Cover ALL documented flags and options for `csplit`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for csplit>"}, {"role": "assistant", "content": "<correct csplit command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<csplit with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use csplit to...")
