# 248 — `swapon` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`swapon`** command in ChatML JSONL format.
...
-a -s -p --all --summary --priority --show

Cover ALL documented flags and options for `swapon`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for swapon>"}, {"role": "assistant", "content": "<correct swapon command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<swapon with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use swapon to...")
