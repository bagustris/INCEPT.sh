# 071 — `lspci` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`lspci`** command in ChatML JSONL format.
...
-v -vv -vvv -k -n -nn -t -s -d -mm -D --verbose --kernel --numeric --tree --slot --device

Cover ALL documented flags and options for `lspci`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for lspci>"}, {"role": "assistant", "content": "<correct lspci command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<lspci with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use lspci to...")
