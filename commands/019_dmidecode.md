# 019 — `dmidecode` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`dmidecode`** command in ChatML JSONL format.
...
-t -s --type --string memory processor system baseboard bios cache connector slot

Cover ALL documented flags and options for `dmidecode`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dmidecode>"}, {"role": "assistant", "content": "<correct dmidecode command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dmidecode with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dmidecode to...")
