# 363 — `scutil` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`scutil`** command in ChatML JSONL format.

## Command Overview

`scutil` — System configuration utility (macOS)
...
--dns --proxy --nwi --get --set ComputerName LocalHostName HostName

Cover ALL documented flags and options for `scutil`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for scutil>"}, {"role": "assistant", "content": "<correct scutil command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<scutil with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use scutil to...")
