# 146 — `nproc` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nproc`** command in ChatML JSONL format.
...
Cover ALL documented flags and options for `nproc`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nproc>"}, {"role": "assistant", "content": "<correct nproc command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nproc with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nproc to...")
