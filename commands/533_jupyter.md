# 533 — `jupyter` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`jupyter`** command in ChatML JSONL format.
...
notebook lab console kernelspec nbconvert --ip --port --no-browser --generate-config

Cover ALL documented flags and options for `jupyter`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for jupyter>"}, {"role": "assistant", "content": "<correct jupyter command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<jupyter with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use jupyter to...")
