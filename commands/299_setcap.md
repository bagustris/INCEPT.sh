# 299 — `setcap` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`setcap`** command in ChatML JSONL format.
...
-r -v --remove --verbose cap_net_bind_service cap_sys_admin cap_net_raw cap_dac_override

Cover ALL documented flags and options for `setcap`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for setcap>"}, {"role": "assistant", "content": "<correct setcap command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<setcap with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use setcap to...")
