# 035 — `mtr` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`mtr`** command in ChatML JSONL format.
...
-r -c -n -w -b -o -s --report --report-cycles --no-dns --report-wide --show-ips --order --psize --json --xml --csv

Cover ALL documented flags and options for `mtr`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for mtr>"}, {"role": "assistant", "content": "<correct mtr command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<mtr with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use mtr to...")
