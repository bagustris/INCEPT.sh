# 378 — `od` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`od`** command in ChatML JSONL format.
...
-A -t -j -N -w --address-radix --format --skip-bytes --read-bytes --width

Cover ALL documented flags and options for `od`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for od>"}, {"role": "assistant", "content": "<correct od command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<od with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use od to...")
