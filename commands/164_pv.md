# 164 — `pv` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`pv`** command in ChatML JSONL format.
...
-p -t -e -r -b -n -q -L --progress --timer --eta --rate --bytes --numeric --quiet --rate-limit

Cover ALL documented flags and options for `pv`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pv>"}, {"role": "assistant", "content": "<correct pv command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pv with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pv to...")
