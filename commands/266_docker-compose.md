# 266 — `docker-compose` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`docker-compose`** command in ChatML JSONL format.
...
up down build start stop restart logs ps exec pull push config rm --build -d --force-recreate --no-deps --scale --remove-orphans -f --file

Cover ALL documented flags and options for `docker-compose`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for docker-compose>"}, {"role": "assistant", "content": "<correct docker-compose command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<docker-compose with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use docker-compose to...")
