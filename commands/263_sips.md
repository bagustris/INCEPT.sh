# 263 — `sips` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`sips`** command in ChatML JSONL format.

## Command Overview

`sips` — Scriptable image processing (macOS)
...
-s -g -z -Z --setProperty --getProperty --resampleWidth --resampleHeight --resampleHeightWidthMax --out

Cover ALL documented flags and options for `sips`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sips>"}, {"role": "assistant", "content": "<correct sips command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sips with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sips to...")
