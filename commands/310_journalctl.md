# 310 — `journalctl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`journalctl`** command in ChatML JSONL format.
...
-u -f -b -p -n --since --until -o -k -r --unit --follow --boot --priority --lines --output --kernel --reverse --no-pager --disk-usage --vacuum-size --vacuum-time --vacuum-files -e --pager-end --catalog -x

Cover ALL documented flags and options for `journalctl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for journalctl>"}, {"role": "assistant", "content": "<correct journalctl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<journalctl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use journalctl to...")
