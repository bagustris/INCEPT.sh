# 087 — `history` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`history`** command in ChatML JSONL format.
...
-c -d -a -r -w -n -p -s --clear --delete

Cover ALL documented flags and options for `history`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for history>"}, {"role": "assistant", "content": "<correct history command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<history with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use history to...")
