# 044 — `cp` — TIER 2 — BENCHMARK REINFORCEMENT

> **200 examples** | This command PASSES the benchmark but needs reinforcement to maintain accuracy.

## Task

Generate exactly **200** training examples for the **`cp`** command in ChatML JSONL format.
...
- `-a`: Archive mode; equivalent to -dR --preserve=all (GNU) or -RpP (BSD)
- `--reflink`: Perform lightweight copy-on-write clone if supported by filesystem (auto/always/never)
- `--sparse`: Control creation of sparse files (auto/always/never)
- `-n`: Do not overwrite an existing file

## Additional Flags to Cover

-r -R -i -f -n -v -p -a --preserve -u -l -s --backup

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for cp>"}, {"role": "assistant", "content": "<correct cp command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<cp with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use cp to...")
