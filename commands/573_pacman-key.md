# 573 — `pacman-key` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`pacman-key`** command in ChatML JSONL format.

## Command Overview

`pacman-key` — Pacman keyring management
**Category:** Arch Linux Specific
...
--init --populate --refresh-keys --add --recv-keys --verify --lsign-key

Cover ALL documented flags and options for `pacman-key`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for pacman-key>"}, {"role": "assistant", "content": "<correct pacman-key command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<pacman-key with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use pacman-key to...")
