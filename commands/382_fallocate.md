# 382 — `fallocate` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`fallocate`** command in ChatML JSONL format.
...
-l -n -p -o --length --keep-size --punch-hole --offset

Cover ALL documented flags and options for `fallocate`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for fallocate>"}, {"role": "assistant", "content": "<correct fallocate command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<fallocate with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use fallocate to...")
