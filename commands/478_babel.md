# 478 — `babel` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`babel`** command in ChatML JSONL format.
...
--presets --plugins --config-file --out-dir --out-file --watch --source-maps

Cover ALL documented flags and options for `babel`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for babel>"}, {"role": "assistant", "content": "<correct babel command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<babel with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use babel to...")
