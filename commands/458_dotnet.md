# 458 — `dotnet` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`dotnet`** command in ChatML JSONL format.

## Command Overview

`dotnet` — NET runtime and CLI
**Category:** Programming Languages and Runtimes
...
new build run test publish restore add remove clean pack nuget tool sln --configuration --framework --runtime --output

Cover ALL documented flags and options for `dotnet`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for dotnet>"}, {"role": "assistant", "content": "<correct dotnet command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<dotnet with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use dotnet to...")
