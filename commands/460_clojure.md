# 460 — `clojure` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`clojure`** command in ChatML JSONL format.
...
-M -A -X -T -P --main --aliases --exec --tool --prepare

Cover ALL documented flags and options for `clojure`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for clojure>"}, {"role": "assistant", "content": "<correct clojure command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<clojure with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use clojure to...")
