# 379 — `xxd` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`xxd`** command in ChatML JSONL format.
...
-r -p -i -l -c -s -u --reverse --plain --include --len --cols --seek

Cover ALL documented flags and options for `xxd`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for xxd>"}, {"role": "assistant", "content": "<correct xxd command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<xxd with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use xxd to...")
