# 484 — `yarn` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`yarn`** command in ChatML JSONL format.
...
add remove upgrade install init run test build --dev --peer --optional --exact --tilde --global --frozen-lockfile --production

Cover ALL documented flags and options for `yarn`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for yarn>"}, {"role": "assistant", "content": "<correct yarn command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<yarn with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use yarn to...")
