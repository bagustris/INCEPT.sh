# 464 — `groovy` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`groovy`** command in ChatML JSONL format.
...
-e -c -d -i --eval --compile --debug --indy

Cover ALL documented flags and options for `groovy`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for groovy>"}, {"role": "assistant", "content": "<correct groovy command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<groovy with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use groovy to...")
