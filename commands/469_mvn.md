# 469 — `mvn` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`mvn`** command in ChatML JSONL format.
...
clean compile test package install deploy site -DskipTests -pl -am -P --batch-mode --offline --update-snapshots

Cover ALL documented flags and options for `mvn`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for mvn>"}, {"role": "assistant", "content": "<correct mvn command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<mvn with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use mvn to...")
