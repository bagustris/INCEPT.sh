# 529 — `sqlmap` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`sqlmap`** command in ChatML JSONL format.
...
-u -r --dbs --tables --columns --dump --batch --level --risk --threads --proxy --tor --os-shell

Cover ALL documented flags and options for `sqlmap`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for sqlmap>"}, {"role": "assistant", "content": "<correct sqlmap command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<sqlmap with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use sqlmap to...")
