# 339 — `udevadm` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`udevadm`** command in ChatML JSONL format.
...
info trigger settle monitor control test-builtin hwdb --query --path --name --attribute-walk --action --subsystem-match

Cover ALL documented flags and options for `udevadm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for udevadm>"}, {"role": "assistant", "content": "<correct udevadm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<udevadm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use udevadm to...")
