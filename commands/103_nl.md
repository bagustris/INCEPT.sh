# 103 — `nl` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nl`** command in ChatML JSONL format.
...
-b -i -n -s -w --body-numbering --line-increment --number-format --number-separator --number-width

Cover ALL documented flags and options for `nl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nl>"}, {"role": "assistant", "content": "<correct nl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nl to...")
