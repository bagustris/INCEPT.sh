# 271 — `helm` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`helm`** command in ChatML JSONL format.
...
install uninstall upgrade rollback list repo search show create package template lint status history dependency env -n --namespace --values -f --set --version --create-namespace --wait --timeout --dry-run

Cover ALL documented flags and options for `helm`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for helm>"}, {"role": "assistant", "content": "<correct helm command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<helm with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use helm to...")
