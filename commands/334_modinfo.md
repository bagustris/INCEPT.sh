# 334 — `modinfo` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`modinfo`** command in ChatML JSONL format.
...
-a -d -l -n -p --author --description --license --filename --parm

Cover ALL documented flags and options for `modinfo`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for modinfo>"}, {"role": "assistant", "content": "<correct modinfo command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<modinfo with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use modinfo to...")
