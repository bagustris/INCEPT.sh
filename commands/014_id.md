# 014 — `id` — TIER 1 — BENCHMARK CRITICAL

> **500 examples** | This command is FAILING or WEAK in the 100-question benchmark. Maximum training examples needed.

## Task

Generate exactly **500** training examples for the **`id`** command in ChatML JSONL format.
...
-u -g -G -n -r --user --group --groups --name --real

Cover ALL documented flags and options for `id`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for id>"}, {"role": "assistant", "content": "<correct id command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<id with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use id to...")
