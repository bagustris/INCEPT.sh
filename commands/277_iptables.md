# 277 — `iptables` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`iptables`** command in ChatML JSONL format.
...
-A -D -I -L -F -P -N -X -Z -t -p -s -d --dport --sport -j -i -o -m --append --delete --insert --list --flush --policy --new-chain --delete-chain --zero --table --protocol --source --destination --jump --in-interface --out-interface --match nat filter mangle raw ACCEPT DROP REJECT LOG SNAT DNAT MASQUERADE

Cover ALL documented flags and options for `iptables`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for iptables>"}, {"role": "assistant", "content": "<correct iptables command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<iptables with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use iptables to...")
