# 499 — `localectl` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`localectl`** command in ChatML JSONL format.
...
status set-locale list-locales set-keymap list-keymaps set-x11-keymap

Cover ALL documented flags and options for `localectl`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for localectl>"}, {"role": "assistant", "content": "<correct localectl command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<localectl with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use localectl to...")
