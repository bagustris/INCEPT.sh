# 090 — `ln` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`ln`** command in ChatML JSONL format.
...
-s -f -v -i -r --symbolic --force --relative

Cover ALL documented flags and options for `ln`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for ln>"}, {"role": "assistant", "content": "<correct ln command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<ln with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use ln to...")
