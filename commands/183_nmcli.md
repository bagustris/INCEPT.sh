# 183 — `nmcli` — TIER 3 — IMPORTANT COMMON COMMAND

> **100 examples** | This command is widely used in practice but not directly tested in the benchmark.

## Task

Generate exactly **100** training examples for the **`nmcli`** command in ChatML JSONL format.
...
general networking radio connection device agent monitor status show add modify delete up down reload

Cover ALL documented flags and options for `nmcli`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for nmcli>"}, {"role": "assistant", "content": "<correct nmcli command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<nmcli with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use nmcli to...")
