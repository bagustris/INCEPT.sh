# 577 — `updatedb` — TIER 4 — NICHE/SPECIALIZED

> **20 examples** | This command is specialized/niche. Minimal coverage to establish basic competence.

## Task

Generate exactly **20** training examples for the **`updatedb`** command in ChatML JSONL format.
...
--localpaths --prunepaths --output --verbose

Cover ALL documented flags and options for `updatedb`. Think through the full man page.

## Example Output Lines

```jsonl
{"messages": [{"role": "system", "content": "ubuntu 24.04 bash non-root"}, {"role": "user", "content": "<natural language request for updatedb>"}, {"role": "assistant", "content": "<correct updatedb command>"}]}
{"messages": [{"role": "system", "content": "fedora 39 zsh non-root"}, {"role": "user", "content": "<different phrasing>"}, {"role": "assistant", "content": "<updatedb with different flags>"}]}
```
...
- [ ] Natural language is diverse (not just "use updatedb to...")
